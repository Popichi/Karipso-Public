...
bitV *newVec(uint64_t vectorLength)
{
...
{
    for (uint64_t i = 0; i <= (((bitVector->l) / 8) + 1); i++) 
    {
        bitVector->v[i] |= 0xFF;
    }
    return;
...
//@return void.
void setBit(bitV* bitVector, uint64_t desiredBit)
{
...
//@return void.
void clrBit(bitV* bitVector, uint64_t desiredBit)
{
    bitVector->v[(desiredBit / 8)] &= ~(0x01 << (desiredBit % 8));
    return;
}
...
uint8_t valBit(bitV* bitVector, uint64_t desiredBit)
{
...
uint64_t lenVec(bitV* bitVector)
{
    return bitVector->l;
}

// Increases the size of our bit vector and initializes the new bits to 0.
bitV *reallocVec(bitV *bitVector)
{
    if (bitVector != NIL)
    {
        uint8_t *temporaryPointer = NIL;
        uint64_t originalSize = bitVector->l;
        bitVector->l = (bitVector->l + 16384);
        temporaryPointer = (uint8_t*)realloc(bitVector->v, sizeof(uint8_t) * bitVector->l);
        if (temporaryPointer != NIL)
        {
            bitVector->v = temporaryPointer;
            for (uint64_t i = originalSize; i < (bitVector->l); i++)
            {
                clrBit(bitVector, i);
            }
            return bitVector;
